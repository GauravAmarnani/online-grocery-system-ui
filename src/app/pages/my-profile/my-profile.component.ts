...
export class MyProfileComponent implements OnInit {
  ...
  loadUserProfile(): void {
    if (this.userEmail) {
      this.http.get<any>(`https://onlinegrocerybackend.gauravamarnani.in/api/customers/profile/${this.userEmail}`).subscribe({
        next: (data) => {
  ...
  onSubmit() {
    if (this.profileForm.valid && this.userEmail) {
      ...
      this.http.put<any>(`https://onlinegrocerybackend.gauravamarnani.in/api/customers/profile/${this.userEmail}`, updatedProfile).subscribe({
        next: (response) => {
