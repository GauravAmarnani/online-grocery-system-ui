...
export class CartComponent implements OnInit {
  ...
  loadCartDetails(): void {
    ...
    if (storedCart) {
      ...
      if (productIds.length > 0) {
        this.http.get<Product[]>('http://localhost:8083/products').subscribe(
          (products) => {
  ...
  increaseQuantity(item: { product: Product, quantity: number }): void {
    ...
    this.http.get<Product>(`http://localhost:8083/products/${item.product.id}`).subscribe(
      (product) => {
  ...
  placeOrder(): void {
    ...
    if (storedCart) {
      ...
      this.http.post(`http://localhost:8083/order/place-order/${customerId}`, cart).subscribe({
        next: (response: any) => {
