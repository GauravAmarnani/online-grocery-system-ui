...
export class CartComponent implements OnInit {
  ...
  increaseQuantity(item: { product: Product, quantity: number }): void {
    ...
    this.http.get<Product>(`http://onlinegrocerybackend.gauravamarnani.in:8083/products/${item.product.id}`).subscribe(
      (product) => {
  ...
  placeOrder(): void {
    ...
    if (storedCart) {
      ...
      this.http.post(`http://onlinegrocerybackend.gauravamarnani.in:8083/order/place-order/${customerId}`, cart).subscribe({
        next: (response: any) => {
